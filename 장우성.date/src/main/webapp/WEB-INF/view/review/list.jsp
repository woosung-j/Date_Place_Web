    			...
    			console.log(reviews)
       			if(reviews.length) {
        			...
        			$.ajax({
			    		...
			    		method:'get',
			    		success: data => {
			    			$('#reviewInfo').append('<p class="text-center"><strong>' + `\${reviews[0].placeName}`  
           						+ '<span style="color: #fb3959">★ ' +`\${data}` +'</span></strong><br/>' 
           						+ '<span style="color: #fb3959">' + `\${reviews.length}` + '</span> 개의 리뷰가 있어요.<br /></p>')
			    		}
	...
	<div class="container">
		<header style="padding-top: 80px">
			<nav class="row navbar bg-light text-center align-middle fixed-top">
				<a href="javascript:window.history.back();" class="col btn"><i
		...
		<footer style="padding-top: 100px">
			<nav
				class="row navbar fixed-bottom text-center justify-content-center bg-light">
				<a type="button" class="col-12 btn w-100 font-gamja-flower"
					href="<%=request.getContextPath()%>/review/add"
					style="display: block"> 다녀온 리뷰 쓰기 <i
